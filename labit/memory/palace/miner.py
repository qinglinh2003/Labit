...
def _score_importance(content: str) -> tuple:
    """Score a chunk's importance using the general extractor.

    Returns (importance: float, memory_type: str).
    Importance mapping:
      decision/milestone/problem → 5.0
      preference → 4.0
      emotional → 2.0
      unclassified → 3.0 (default)
    """
    IMPORTANCE_MAP = {
        "decision": 5.0,
        "milestone": 5.0,
    ...
    try:
        memories = extract_memories(content, min_confidence=0.3)
        if memories:
            best = max(memories, key=lambda m: IMPORTANCE_MAP.get(m["memory_type"], 3.0))
            mtype = best["memory_type"]
            return IMPORTANCE_MAP.get(mtype, 3.0), mtype
    except Exception:
        pass
    return 3.0, ""
