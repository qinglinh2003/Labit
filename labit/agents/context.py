...
class ContextBuilder:
    ...
    def build(self, task: TaskSpec, *, project_name: str | None = None) -> ContextPack:
        ...
        if resolved_project:
            ...
            hypothesis_dir = self.paths.vault_projects_dir / resolved_project / "hypotheses"
            memory.open_hypotheses = [
                {"id": path.stem, "path": str(path.relative_to(self.paths.root))}
                for path in sorted(hypothesis_dir.glob("h*.yaml"))
            ][:10]
